 ...
 * Author: BM
 *
 * Created on 08 AralÄ±k 2014 Pazartesi, 14:59
 */
...
void buble_down(struct heap *heap, int index){
    ...
    while((sol <heap->eleman_sayisi &&                  // heap Ã¶zelliÄŸini kontrol et
            heap->dizi[index].key < heap->dizi[sol].key) || 
           (sag <heap->eleman_sayisi && 
            heap->dizi[index].key < heap->dizi[sag].key)){
        if(sag>=heap->eleman_sayisi || heap->dizi[sol].key >heap->dizi[sag].key){ // saÄŸ child yoksa veya sol child saÄŸdan bÃ¼yÃ¼kse
        ...
        else{       // saÄŸ child var ve soldan bÃ¼yÃ¼k
